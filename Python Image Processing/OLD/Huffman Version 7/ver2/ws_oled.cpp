...
#ifdef DEBUG_PIN
void WSOled::DEBUG_ON() {
    digitalWrite(DEBUG_PIN, HIGH);
}

void WSOled::DEBUG_OFF() {
    digitalWrite(DEBUG_PIN, LOW);
}
#endif

void WSOled::oledWriteReg(uint8_t reg) {
    digitalWrite(DC, 0);
    digitalWrite(CS, 0);
    SPI.transfer(reg);
    digitalWrite(CS, 1);
...
void WSOled::oledWriteData(uint8_t dataOut) {
    digitalWrite(DC, 1);
    digitalWrite(CS, 0);
...
int WSOled::begin(char CS, char DC, char RST, uint16_t displayWidth, uint16_t displayHeight) {
    // Init Arduino pins
    pinMode(CS, OUTPUT);
    pinMode(RST, OUTPUT);
    pinMode(DC, OUTPUT);
...
void WSOled::setPixel(uint16_t x, uint16_t y, uint16_t color) {
    oledWriteReg(0x15);
    oledWriteData(x);
    oledWriteData(x);
    oledWriteReg(0x75);
